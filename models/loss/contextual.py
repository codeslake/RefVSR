...
def compute_l2_distance(x, y):
    ...
    # dist = (torch.sum(y_vec ** 2, dim=1, keepdim=True) - (2 * y_vec.transpose(1, 2) @ x_vec) + torch.sum(x_vec ** 2, dim=1, keepdim=True).transpose(1, 2)).transpose(1, 2).reshape(N, H*W, H*W).clamp(min=0.)
...
class ContextualLoss(nn.Module):
    ...
    def forward(self, x, y):
        ...
        if self.is_CoBi:
            return contextual_bilateral_loss(x, y, band_width=self.band_width, loss_type=self.loss_type)
        else:
